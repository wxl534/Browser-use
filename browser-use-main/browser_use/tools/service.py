...
def _detect_image_boundaries(screenshot_bytes: bytes, min_area: int = 10000, 
                              edge_threshold_low: int = 50, 
                              edge_threshold_high: int = 150) -> list[dict]:
	"""
	检测截图中的图片边界
	...
	try:
		...
		logger.error(f"❌ 图片边界检测失败：{type(e).__name__}: {e}")
		return []
...
""".strip()
		...
		@self.registry.action(
			'Smart screenshot: Automatically detect and crop images from background. '
			'If target_image_index is provided, crops only that specific image. '
			'Saves cropped images to specified folder. '
			'Requires: opencv-python, pillow, numpy (run: uv add opencv-python pillow numpy)',
			param_model=SmartScreenshotAction,
		)
		async def smart_screenshot(
			params: SmartScreenshotAction,
			browser_session: BrowserSession,
			file_system: FileSystem,
		):
			"""智能截图：自动识别并裁剪图片区域"""
			try:
				...
				# 2. 检测图片边界
				image_regions = _detect_image_boundaries(
					screenshot_bytes, 
				...
				if params.output_folder:
					output_dir = file_system.get_dir() / params.output_folder
					output_dir.mkdir(parents=True, exist_ok=True)
				else:
					output_dir = file_system.get_dir()
				
				# 5. 根据参数决定裁剪策略
				if params.target_image_index is not None:
					# 裁剪特定图片
					if params.target_image_index >= len(image_regions):
						return ActionResult(
							error=f'请求的图片索引 {params.target_image_index} 超出范围 (0-{len(image_regions)-1})',
							extracted_content=f'Index {params.target_image_index} out of range'
						)
					
					region = image_regions[params.target_image_index]
					...
					# 保存单张图片
					file_name = params.file_name or 'cropped_image'
					if not file_name.lower().endswith('.png'):
						file_name = f'{file_name}.png'
					file_name = FileSystem.sanitize_filename(file_name)
					...
					result = f'裁剪图片 #{params.target_image_index} 保存到 {file_name}'
					logger.info(f'📸 {result}. 区域：{region}')
					
					return ActionResult(
						extracted_content=result,
						long_term_memory=f'{result}. 路径：{file_path}',
						attachments=[str(file_path)]
					)
					...
					for idx, region in enumerate(image_regions):
						...
						if params.file_name:
							file_name = f'{params.file_name}_{idx}'
						...
						logger.info(f'📸 保存裁剪图片 #{idx}: {file_name}')
					
					result = f'检测到 {len(image_regions)} 张图片，保存到 {saved_files}'
					logger.info(f'📸 {result}')
					
					return ActionResult(
						extracted_content=result,
						long_term_memory=f'批量裁剪 {len(image_regions)} 张图片',
						attachments=saved_files
					)
			
			except Exception as e:
				logger.error(f"❌ 智能截图失败：{type(e).__name__}: {e}")
				return ActionResult(
					error=f'智能截图失败：{str(e)}',
					extracted_content=f'Smart screenshot failed: {str(e)}'
				)
		...
		@self.registry.action(
			'Replace specific text within a file by searching for old_str and replacing with new_str. Use this for targeted edits like updating todo.md checkboxes or modifying specific lines without rewriting the entire file.'
		)
